...
def find_one():
    ...
    username_textarea_button = cv2.imread(
        get_image(
            'username_textarea.png',
            __file__,
            'const_images'
        )
    )
    ...
    assert username_textarea_button is not None, 'Загрузи юзернейм текстареа пидор'
    ...
    for prefab in prefabs:
        ...
        result = cv2.matchTemplate(
            image=image,
            templ=username_textarea_button,
        ...
        if max_val >= 0.8:
            w, h = username_textarea_button.shape[1], username_textarea_button.shape[0]
