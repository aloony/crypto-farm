...
def find_one():
    ...
    character_confirm_button = cv2.imread(
        get_image(
            'confirm_character.png',
            __file__,
            'const_images'
        )
    )
    ...
    assert character_confirm_button is not None, 'Загрузи карактер конфирм пидор'
    ...
    for prefab in prefabs:
        ...
        result = cv2.matchTemplate(
            image=image,
            templ=character_confirm_button,
        ...
        if max_val >= 0.8:
            w, h = character_confirm_button.shape[1], character_confirm_button.shape[0]
